...
void BoundedDepthFirstSearch::Run(struct FDS fds) {
  ...
  UpdateCoverage(ex);
  // Get sockid
  // FILE *fp = fopen("client_fd","r");
  // fread((void *)&sockid,sizeof(int),1,fp);
  // fclose(fp);

  //Get output
  char server_output[1024];
  FILE* fp = fopen("server_output","r");
  fseek (fp , 0 , SEEK_END);
  int lSize = ftell (fp);   
  rewind (fp);
  fread(server_output,1,lSize,fp);
  fclose(fp);
  /* Send Constraints */
  string s="";
  printf("Preparing to send constraints\n");
  ex.path().Serialize(&s);
  int const_size = s.length();
  // char * server_constraints = new char[const_size];
  // bzero(server_constraints,const_size);
  // memcpy(server_constraints,s.c_str(),const_size);
  size_t total_send_length = lSize+const_size+2*sizeof(int);
  // printf("lSize:%d\n",lSize);
  char *send_buffer = (char *)malloc(1024);
  bzero(send_buffer,1024);
  memcpy(send_buffer,&lSize,sizeof(int));
  memcpy(send_buffer+sizeof(int),server_output,lSize);
  memcpy(send_buffer+sizeof(int)+lSize,&const_size,sizeof(int));
  // printf("Const_size:%d\n",const_size);
  memcpy(send_buffer+2*sizeof(int)+lSize,s.c_str(),const_size);
  char server_input[1024];
  fp = fopen("server_input","r");
  // fseek (fp , 0 , SEEK_END);
  // lSize = ftell (fp);   
  // rewind (fp);
  // fread(server_input,1,lSize,fp);
  // fclose(fp);
  int num;
  fscanf(fp,"%d",&num);
  memcpy(send_buffer+2*sizeof(int)+lSize+const_size,&num,sizeof(int));
  for(int i=0;i<num;i++){
    value_t aa;
    fscanf(fp,"%lld",&aa);
    memcpy(send_buffer+3*sizeof(int)+lSize+const_size+i*sizeof(value_t)
      ,&aa,sizeof(value_t));
  }
  // printf("lSize2:%d\n",lSize);  
  
  total_send_length += (sizeof(int)+lSize);
  send_branch_cond(&fds,send_buffer,total_send_length);
  sleep(1);

  DFS(0, max_depth_, ex, fds, max_depth_);
  // DFS(0, ex);
}
...
void BoundedDepthFirstSearch::DFS(size_t pos, int depth, SymbolicExecution& prev_ex, FDS fds, int max_depth) {
  SymbolicExecution cur_ex;
  vector<value_t> input;
  ...
  for (size_t i = pos; (i < path.constraints().size()) && (depth > 0); i++) {
    ...
    /* Send Constraints */
    // string s="";
    // cur_ex.path().Serialize(&s);
    // char * server_constraints = new char[s.length() + 1];
    // strcpy(server_constraints,s.c_str());
    // send_branch_cond(sockid,server_constraints,s.length()+1);
    // printf("const:%s\n",server_constraints);
    ...
    // memcpy(server_constraints,s.c_str(),const_size);
    size_t total_send_length = lSize+const_size+2*sizeof(int);
    char *send_buffer = (char *)malloc(1024*sizeof(int));
    bzero(send_buffer,total_send_length);
    memcpy(send_buffer,&lSize,sizeof(int));
    memcpy(send_buffer+sizeof(int),server_output,lSize);
    memcpy(send_buffer+sizeof(int)+lSize,&const_size,sizeof(int));
    memcpy(send_buffer+2*sizeof(int)+lSize,s.c_str(),const_size);
    vector<value_t> server_input = cur_ex.inputs();
    int len = server_input.size();
    memcpy(send_buffer+2*sizeof(int)+lSize+const_size,&len,sizeof(int));
    for(int i=0;i<len;i++)
      memcpy(send_buffer+2*sizeof(int)+lSize+const_size+sizeof(int)+i*sizeof(value_t),
        &server_input[i],sizeof(value_t));
    total_send_length += (len*sizeof(value_t)+sizeof(int));
    send_branch_cond(&fds,send_buffer,total_send_length);
    // sleep(2);
    ...
    depth--;
    DFS(i+1, depth, cur_ex, fds, max_depth);
  }
  if(depth == max_depth){
    can_die(fds);
  }
...
void RandomInputSearch::Run(struct FDS fds) {
  vector<value_t> input;
...
void RandomSearch::Run(struct FDS fds) {
...
void UniformRandomSearch::Run(struct FDS fds) {
  // Initial execution (on empty/random inputs).
...
void HybridSearch::Run(struct FDS fds) {
  SymbolicExecution ex;
...
void CfgBaselineSearch::Run(struct FDS fds) {
  SymbolicExecution ex;
...
void CfgHeuristicSearch::Run(struct FDS fds) {
