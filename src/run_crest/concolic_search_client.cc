...
bool Search::SolveAtBranch(const SymbolicExecution& ex,
  ...
  vector<const SymbolicPred*> cs(server_path.constraints().begin(),
              server_path.constraints().end());
  ...
  //Add server constraints
  cs.insert(cs.end(),constraints.begin(),constraints.begin()+branch_idx+1);
  printf("Before\n");
...
void BoundedDepthFirstSearch::Run(struct FDS fds) {
  ...
  // Get the output of server    
  // recv_output(sockid,output_server);
  // while (1){
  //   while (num_child > 5){
  //       int status;
  //       if (wait(&status)==0){
  ...
  printf("Receiving Constraints\n");
  int cond_recv_size =0;
  int sockid = fds.fd[0];
  cond_recv_size = recv_constraints(sockid,recv_buffer);
  server_output_size = *(int *)recv_buffer;
  bzero(server_output,1024);
  memcpy(server_output,recv_buffer+sizeof(int),server_output_size);
  // printf("Server Output Size:%d\n",server_output_size);
  int const_size = *(int *)(recv_buffer+sizeof(int)+server_output_size);
  // printf("Const_size:%d\n",const_size);
  string str(recv_buffer+2*sizeof(int)+server_output_size,const_size+1);
  str[const_size] = '\0';
  istringstream iss(str);
  server_path.Parse(iss);
  vector<value_t> inputs;
  int num = *(int*)(recv_buffer+2*sizeof(int)+const_size+server_output_size);
  // printf("NUM:%d\n",num);
  for(int i=0;i<num;i++){
    value_t a = *(value_t*)(recv_buffer+3*sizeof(int)+i*sizeof(value_t)+const_size+server_output_size);
    inputs.push_back(a);
  }
  RunProgram(inputs, &ex);
  UpdateCoverage(ex);
  char client_output[1024];
  bzero(client_output,1024);
  FILE* fp=fopen("client_output","r");
  // obtain file size:
  fseek (fp , 0 , SEEK_END);
  int lSize = ftell (fp);   
  rewind (fp);
  fread(client_output,1,lSize,fp);
  fclose(fp);
  if(send_ans(client_output, server_output,lSize,server_output_size)){
    printf("Outputs Matched\n");
  }else{
    printf("Outputs Do Not Match\n");
    char buf[] = "MisMatch";
    send_output(fds,buf,strlen(buf));
    Run(fds);
    return;
  }
...
void BoundedDepthFirstSearch::DFS(size_t pos, int depth, SymbolicExecution& prev_ex, FDS fds, int max_depth) {
  ...
  const SymbolicPath& path = prev_ex.path();
  char output_client[1024]="";
  // printf("Const Size:%d\n",path.constraints().size());
  // if(pos == 0 && path.constraints().size()==0){
  //   SymbolicExecution ex;
  //   printf("Entered\n");
  //   SolveAtBranch(ex,-1,&input);
  // }
