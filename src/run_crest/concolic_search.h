...
class Search {
 public:
  ...
  virtual void Run(struct FDS) = 0;

  // Socket connections
  int sockid;
  char server_output[1024];
  SymbolicPath server_path;
  int server_output_size;
...
class BoundedDepthFirstSearch : public Search {
 public:
  ...
  virtual void Run(struct FDS);

 private:
  int max_depth_;

  void DFS(size_t pos, int depth, SymbolicExecution& prev_ex, FDS, int max_depth);
};
