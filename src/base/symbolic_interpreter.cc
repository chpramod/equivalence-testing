...
void SymbolicInterpreter::WriteZ(id_t id, addr_t addr) {
  // FILE *fp = fopen("server_output","w");
  string s="";
  printf("Write Symbolic constrainst in file\n");
  mem_[addr]->Serialize(&s);
  string filename;
  if(id == 1)
    filename = "server_output";
  else
    filename = "client_output";
  std::ofstream out(filename.c_str());
  out << s;
  out.close();
  cout << filename << " s::: " << s << endl;
  // int len = s.length();
  // char *send_buffer = (char *)malloc(len+1);
  // bzero(send_buffer,len+1);
  // memcpy(send_buffer,s.c_str(),len+1);
  // printf("Send_buffer len:%u %u\n",len,strlen(send_buffer));
  // printf("Char written:%d\n",fprintf(fp,"%s",send_buffer));
  // fclose(fp);
}
...
value_t SymbolicInterpreter::NewInput(type_t type, addr_t addr) {
  ...
  value_t ret = 0;
  // printf("num_inputs_:%d ex_in:%d\n",num_inputs_,ex_.inputs().size());
